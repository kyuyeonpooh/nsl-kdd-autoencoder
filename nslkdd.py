...
def get_service_list(dirname='data', filename='service.txt'):
...
def get_flag_list(dirname='data', filename='flag.txt'):
...
def to_numeric(data_frame, service_list, flag_list, test=False, attack=False, save=True):
    ...
    if not test:
        if not attack:
            ...
            print('Data size before normal data extraction: {0}'.format(df.shape))
            # index 41: label (index number starts fro 0)
            df_normal_copy = df[df[41] == 'normal'].copy()
            df_normal = df_normal_copy.sample(n=int(df_normal_copy.shape[0] * 0.99),
                                              random_state=1398)
            df_attack = df[df[41] != 'normal'].sample(n=int(df_normal_copy.shape[0] * 0.01) + 1,
                                                      random_state=1398)
            print('Number of normal records:', len(df_normal))
            print('Number of attack records:', len(df_attack))
            df = pd.concat((df_normal, df_attack))
            print('Data size of concatenated data: {0}'.format(df.shape))
    ...
    if save:
        ...
        if not test:
            if not attack:
                ...
                df.to_csv('csv/data_frame_train_numeric.csv', sep=',')
            else:
                print('Saving file: csv/data_frame_train_with_attack_numeric.csv')
                df.to_csv('csv/data_frame_train_with_attack_numeric.csv', sep=',')
        else:
            print('Saving file: csv/data_frame_test_numeric.csv')
            df.to_csv('csv/data_frame_test_numeric.csv', sep=',')

    return df


def to_machine_readable(data_frame, service_list, flag_list, test=False, attack=False, save=True):
    df = data_frame
    sc = MinMaxScaler()
    ...
    # if not test:
    #     print('Before dropping duplicates: {0}'.format(df_final.values.shape))
    ...
    if save:
        ...
        if not test:
            if not attack:
                ...
                print('Saving file: csv/data_frame_train_with_attack_final.csv')
                df_final.to_csv('csv/data_frame_train_with_attack_final.csv', sep=',', index=False)
        else:
    ...
    if not test:
        if not attack:
            with h5py.File('hdf5/train/nsl_kdd_train.hdf5', 'w') as hdf:
                print('Saving file: hdf5/train/nsl_kdd_train.hdf5')
                hdf['data'] = df_final.values[:]
        else:
            with h5py.File('hdf5/train_with_attack/nsl_kdd_train_with_attack.hdf5', 'w') as hdf:
                print('Saving file: hdf5/train_with_attack/nsl_kdd_train_with_attack.hdf5')
                hdf['data'] = df_final.values[:]
    else:
        with h5py.File('hdf5/test/nsl_kdd_test.hdf5', 'w') as hdf:
            print('Saving file: hdf5/test/nsl_kdd_test.hdf5')
            hdf['data'] = df_final.values[:]
            hdf['label'] = df_label[:]
